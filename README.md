...
As described in the previous section, Islandora Datastream CRUD uses underscores (`_`) to in filenames to separate the two parts of each object's PID and the datastream ID from each other. If your PIDs contain underscores, or your datastream IDs contain underscores, you should use the `--filename_separator` option with `islandora_datastream_crud_fetch_datastreams` and `islandora_datastream_crud_push_datastreams` so that your PIDs and DSIDs are unambiguously added to/parsed from their filenames. Any character other than `:` and `/` can be used with this option. For example:
